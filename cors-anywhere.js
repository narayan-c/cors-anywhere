...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    req.url = '/' + Buffer.from(req.url.slice(1), 'base64').toString('utf8');
    console.log("Got new request for resource: " + req.url);
    ...
    if (req.method === 'OPTIONS') {
      console.log("pre-flight request, replying with 200 OK");
    ...
    if (req.url.slice(1) === "status") {
      console.log("Status request, replying with 200 OK");
      res.writeHead(200, 'Happy Koala', cors_headers);
      res.end('Happy Koala');
      return;
    }
    ...
    if (!location) {
      ...
      if (/^\/https?:\/[^/]/i.test(req.url)) {
        console.log("Icy Radish: CORS Anywhere is likely behind a proxy that normalized the URL!");
        res.writeHead(400, 'Icy Radish', cors_headers);
        res.end('Icy Radish');
        return;
      }
    ...
    if (location.host === 'iscorsneeded') {
      ...
      // are not necessary.
      console.log("CORS not needed for this resource");
    ...
    if (location.port > 65535) {
      ...
      console.log("Half Corn: Port is higher than 65535");
      res.writeHead(400, 'Half Corn', cors_headers);
      res.end('Half Corn');
      return;
    }
    ...
    if (!/^\/https?:/.test(req.url) && !isValidHostName(location.hostname)) {
      ...
      console.log("Natural Pumpkin: Invalid hosts");
      res.writeHead(404, 'Natural Pumpkin', cors_headers);
      res.end('Natural Pumpkin');
      return;
    }
    */

    /*if (!hasRequiredHeaders(req.headers)) {
      console.log("Sweet Ginger: required headers are not present");
      res.writeHead(400, 'Sweet Ginger', cors_headers);
      res.end('Sweet Ginger');
      return;
    }*/

    var origin = req.headers.origin || '';
    if (corsAnywhere.originBlacklist.indexOf(origin) >= 0) {
      console.log("Sleepy Onion: Origin in blacklist");
      res.writeHead(403, 'Sleepy Onion', cors_headers);
      res.end('Sleepy Onion');
      return;
    }

    if (corsAnywhere.originWhitelist.length && corsAnywhere.originWhitelist.indexOf(origin) === -1) {
      console.log("Curious Cucumber: Origin nor in whitelist");
      res.writeHead(403, 'Curious Cucumber', cors_headers);
      res.end('Curious Cucumber');
      return;
    }

    var rateLimitMessage = corsAnywhere.checkRateLimit && corsAnywhere.checkRateLimit(origin);
    if (rateLimitMessage) {
      console.log("Anxious Potato: Rate limit exceeded");
      res.writeHead(429, 'Anxious Potato', cors_headers);
      res.end('Anxious Potato');
      return;
    }
    ...
    if (corsAnywhere.redirectSameOrigin && origin && location.href[origin.length] === '/' &&
      ...
      console.log("redirected same origin request");
      cors_headers.vary = 'origin';
      cors_headers['cache-control'] = 'private';
      cors_headers.location = location.href;
      res.writeHead(301, 'Wet Apple', cors_headers);
      res.end();
      return;
    }
